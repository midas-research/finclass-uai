...
class FinNLPCL(nn.Module):
    ...
    def __init__(
        ...
        self.linear3_price = nn.Linear(self.lstm2_outshape, 256)
        self.linear4_price = nn.Linear(256, 2)

        self.linear3_vol = nn.Linear(self.lstm2_outshape, 256)
        self.linear4_vol = nn.Linear(256, 1)
    ...
    def forward(self, sentence_feats, len_tweets):
        ...
        x_price = self.drop(self.relu(self.linear3_price(x)))
        x_price = self.linear4_price(x_price)

        x_vol = self.drop(self.relu(self.linear3_vol(x)))
        x_vol = self.linear4_vol(x_vol)

        return x_vol, x_price
