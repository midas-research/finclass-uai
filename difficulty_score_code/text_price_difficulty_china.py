...
model =  transformers.AutoModelForSequenceClassification.from_pretrained('/content/drive/MyDrive/MIDAS_RL_NLP/saved_models/bert_chinese')
...
def windowed_data_to_difficulty_score(windowed_data):
    ...
    for date in date_list:
        ...
        price_feature.append([date_data['Open'].values[0],date_data['High'].values[0],date_data['Low'].values[0],date_data['Close'].values[0]])
...
cwd = os.getcwd()
path_tweet_data  = "Enter path to text data here"
...
with open('Enter path to train_split_chinese.pkl here','rb') as f:
...
with open("text_price_difficulty_chinese.pkl", "wb") as f:
    pkl.dump(text_price_difficulty_train, f)

with open("text_difficulty_chinese.pkl", "wb") as f:
    pkl.dump(text_difficulty_train, f)

with open("price_difficulty_chinese.pkl", "wb") as f:
